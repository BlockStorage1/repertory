...
void webserver::sweet_kill() { stop(); }

void webserver::request_completed(void *cls, struct MHD_Connection *connection, void **con_cls,
...
bool webserver::register_resource(const std::string &resource, http_resource *hrm, bool family) {
  if (single_resource && ((resource != "" && resource != "/") || !family)) {
    throw std::invalid_argument("The resource should be '' or '/' and be marked as family when "
                                "using a single_resource server");
  }
...
bool webserver::start(bool blocking) {
  ...
  if (digest_auth_random != "")
    iov.push_back(gen(MHD_OPTION_DIGEST_AUTH_RANDOM, digest_auth_random.size(),
  ...
  daemon = NULL;
  if (bind_address == 0x0) {
    daemon = MHD_start_daemon(start_conf, port, &policy_callback, this, &answer_to_connection, this,
                              MHD_OPTION_ARRAY, &iov[0], MHD_OPTION_END);
  } else {
    daemon = MHD_start_daemon(start_conf, 1, &policy_callback, this, &answer_to_connection, this,
                              MHD_OPTION_ARRAY, &iov[0], MHD_OPTION_SOCK_ADDR, bind_address,
                              MHD_OPTION_END);
  }
...
void webserver::unban_ip(const string &ip) { bans.erase(ip); }

void webserver::disallow_ip(const string &ip) { allowances.erase(ip); }
...
MHD_Result webserver::post_iterator(void *cls, enum MHD_ValueKind kind, const char *key,
...
void webserver::upgrade_handler(void *cls, struct MHD_Connection *connection, void **con_cls,
                                int upgrade_socket) {}
...
const std::shared_ptr<http_response> webserver::not_found_page(details::modded_request *mr) const {
  if (not_found_resource != 0x0) {
    ...
    return std::shared_ptr<http_response>(
        new string_response(NOT_FOUND_ERROR, http_utils::http_not_found));
  }
}

const std::shared_ptr<http_response>
webserver::method_not_allowed_page(details::modded_request *mr) const {
  if (method_not_allowed_resource != 0x0) {
    return method_not_allowed_resource(*mr->dhr);
  } else {
    return std::shared_ptr<http_response>(
        new string_response(METHOD_ERROR, http_utils::http_method_not_allowed));
  }
}

const std::shared_ptr<http_response> webserver::internal_error_page(details::modded_request *mr,
                                                                    bool force_our) const {
  if (internal_error_resource != 0x0 && !force_our) {
    return internal_error_resource(*mr->dhr);
  } else {
    return std::shared_ptr<http_response>(
        new string_response(GENERIC_ERROR, http_utils::http_internal_server_error, "text/plain"));
  }
}

MHD_Result webserver::requests_answer_first_step(MHD_Connection *connection,
  ...
  if (post_process_enabled &&
      (0x0 != encoding &&
       ((0 == strncasecmp(http_utils::http_post_encoding_form_urlencoded.c_str(), encoding,
                          http_utils::http_post_encoding_form_urlencoded.size())) ||
        (0 == strncasecmp(http_utils::http_post_encoding_multipart_formdata.c_str(), encoding,
...
MHD_Result webserver::finalize_answer(MHD_Connection *connection,
...
MHD_Result webserver::complete_request(MHD_Connection *connection,
...
MHD_Result webserver::answer_to_connection(void *cls, MHD_Connection *connection, const char *url,
  ...
  if (mr->second != false) {
    return static_cast<webserver *>(cls)->requests_answer_second_step(
